<template>
    <div>
      <input type="checkbox" id="darkmode-toggle" v-model="isDarkMode" @change="toggleDarkMode" />
  ...
  label {
    width: 60px; /* Regular width */
    height: 35px; /* Regular height */
    position: relative;
    display: block;
    background: #ebebeb;
    border-radius: 25px; /* Regular border-radius */
  ...
  label:after {
    content: "";
    width: 30px; /* Regular width */
    height: 30px; /* Regular height */
    position: absolute;
    top: 2.5px;
    left: 2.5px;
    background: linear-gradient(180deg,#ff8989,#d80b0b);
    border-radius: 25px; /* Regular border-radius */
  ...
  input:checked + label:after {
    left: 57.5px; /* Adjusted position */
    transform: translateX(-100%);
    background: linear-gradient(180deg,#777,#3a3a3a);
  }
  
  label:active:after {
    width: 70px; /* Regular width */
  }
